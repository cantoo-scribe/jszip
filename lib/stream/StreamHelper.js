...
var createWebStreamOutputAdapter = require("./WebStreamOutputAdapter");
...
StreamHelper.prototype = {
    ...
    accumulateSync : function () {
        ...
        if (!ended) {
            // an asynchronous source sneaked into the chain: everything
            // synchronous already happened, the remaining work is scheduled
            // on the event loop and can't be awaited here.
    ...
    toWebStream : function (updateCb) {
        ...
        if (this._outputType !== "uint8array" && this._outputType !== "nodebuffer") {
            // binary chunks only: a stream of strings/blobs has no obvious
            // use case and byte-based consumers (Response, pipeTo...) would
            // choke on it.
            throw new Error(this._outputType + " is not supported by this method");
        }
